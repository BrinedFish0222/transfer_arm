...
class _GameScriptFlowPageState extends State<GameScriptFlowPage> {
  ...
  Widget build(BuildContext context) {
    return ChangeNotifierProvider<GameScriptModel>(
      create: (_) => GameScriptModel(data: widget.gameScript),
        ...
        builder: (context, model, _) {
          return Scaffold(
            ...
            floatingActionButton: FloatingActionButton(
                onPressed: () => _buildAddScriptFlow(model), child: const Icon(Icons.add)),
          );
        }
      ),
    );
  ...
  _buildAddScriptFlow(GameScriptModel model) async {
    GameScriptFlow? gameScriptFlow =
        await CommonWidgetUtils.buildBottomSheet<GameScriptFlow>(
            context: context,
            isDismissible: false,
            heightRadio: 0.7,
            child: const Padding(
...
class GsFlowWaitWidget extends StatelessWidget {
  const GsFlowWaitWidget({Key? key, this.width, required this.flow}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    return Row(children: [
      Text("等待时间（毫秒值）：${flow.waitMillisecond?.toString() ?? ''}"),
      if (width != null)
...
class GsFlowMouseWidget extends StatelessWidget {
  const GsFlowMouseWidget({Key? key, this.width, required this.flow}) : super(key: key);
